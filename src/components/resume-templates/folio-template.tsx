...
export const FolioTemplate: React.FC<TemplateProps> = ({ data, accentColor: accentColorProp, fontSize }) => {
  ...
  const palette = { accent: '#37474F', accentSoft: '#ECEFF1', text: '#263238', bg: '#FFFFFF' };
  ...
  return (
    <div className={cn("bg-white text-[var(--fs-body)] w-full h-full flex font-body-open-sans", fontSize === 'sm' ? 'text-sm' : fontSize === 'lg' ? 'text-base' : '')}>
      <aside className="w-[33%] p-6 flex flex-col gap-6" style={{ backgroundColor: palette.accentSoft }}>
        {personalInfo.photoUrl && (
          <img src={personalInfo.photoUrl} alt={fullName} className="w-32 h-32 object-cover mx-auto mt-2 border-4 border-white shadow-md" data-ai-hint="person face" />
        )}
      ...
      <main className="w-[67%] p-8 overflow-y-auto">
        <header className="mb-6">
            <h1 className="text-[2.5rem] leading-tight font-bold">{fullName || 'Your Name'}</h1>
