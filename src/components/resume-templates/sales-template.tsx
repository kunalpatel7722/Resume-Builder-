    ...
    <div className={cn("bg-white text-[var(--fs-body)] w-full h-full flex font-body-mulish", fontSize === 'sm' ? 'text-sm' : fontSize === 'lg' ? 'text-base' : '')}>
      <main className="w-[73%] p-8 overflow-y-auto space-y-6">
        ...
        <Section title="Summary" icon={Target}>
          <p className="leading-relaxed">{summary || "Your professional summary will appear here. Focus on your sales achievements, revenue growth, and relationship-building skills."}</p>
        </Section>
            ...
            <div className="space-y-2">
              {KPIs.map(kpi => (
                  <div key={kpi.id} className="text-center bg-white p-2 rounded shadow">
                    <p className="text-xl font-bold" style={{color: accentColor}}>{kpi.description.match(/(\d+%?)/)?.[0] || 'Value'}</p>
